...
def get_trigger_vocabs(file_paths):
    trigger_vocab = vocab.Vocab()
    # keep 'neutral' in index-0
    # emotion_vocab.word2index('neutral', train=True)
    # global speaker_vocab, emotion_vocab
    for file_path in file_paths:
        ...
        for row in tqdm(data.iterrows(),
            ...
            meta = row[1]
            # print(row)
            # if meta['Trigger'].isnan():
            #     meta['Trigger'] = 0    

            emotion = int(meta['Trigger'])
            trigger_vocab.word2index(emotion, train=True)
    if CONFIG['local_rank'] in [-1, 0]:
        torch.save(trigger_vocab.to_dict(), CONFIG['emotion_vocab'])
    logging.info('total {} emotions'.format(len(trigger_vocab)))


def build_dataset_trigger(dialogues,train=False):
    ret_utterances = []
    ret_labels = []
    for dialogue in dialogues:
        ...
        for idx, turn_data in enumerate(dialogue):
            ...
            text_with_speaker = turn_data['speaker'] + ':' + turn_data['text'] + 'feels '+turn_data['emotion']
            token_ids = tokenizer(text_with_speaker)['input_ids'][1:]
            utterance_ids.append(token_ids)


            if turn_data['trigger'] < 0:
                continue
            ...
            query_idx = idx
            prompt = dialogue[query_idx]['speaker'] + ' is possibly <mask>'
            ...
            if train and idx > 3 and torch.rand(1).item() < 0.2:
                ...
                ret_labels.append(dialogue[query_idx]['trigger'])
    #         print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
    #         print(dialogue[query_idx]['trigger'])
    #         print(turn_data['trigger'])
    #         print(input_ids)
    #         print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
...
def load_trigger(file_path):
    ...
    for row in tqdm(data.iterrows(),
        ...
        emotion_idx = emotion_vocab.word2index(int(meta['Trigger']))
        turn_data = {}
        turn_data['speaker'] = str(speaker)
        speaker_vocab.word2index(speaker, train=True)
        turn_data['text'] = str(text)
        turn_data['emotion']=str(emotion)
        turn_data['trigger'] = emotion_idx
