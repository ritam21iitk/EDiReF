...
our_training_path = "../Data/MASAC_train_efr_hypo.csv"
our_testing_path = "../Data/MASAC_val_efr_hypo.csv"
save_path = "../Pickles_MASAC_EFR_Hypo/"

our_training_csv = pd.read_csv(our_training_path)
our_testing_csv = pd.read_csv(our_testing_path)

all_data = pd.concat([our_training_csv,our_testing_csv])
...
all_ID= all_data["Dialogue_Id"]

all_utterances = all_data["Utterance"]

all_emotions = all_data["Emotion_name"]

all_speakers = all_data["Speaker"]

all_annotations = all_data["Annotate(0/1)"]
...
from transformers import AutoTokenizer, BertModel,AutoModelForMaskedLM, AutoModelForTokenClassification
from transformers import pipeline
import torch

pipe = pipeline("fill-mask", model="l3cube-pune/hing-bert")
tokenizer = AutoTokenizer.from_pretrained("l3cube-pune/hing-bert")
he_model = AutoModelForTokenClassification.from_pretrained("l3cube-pune/hing-bert-lid")
...
for utt in tqdm.tqdm(idx2utt.values()):
    # print(utt)
    pp_utt = nu.preprocess_text(utt)
    if pp_utt != "pad":
      marked_text = "[CLS] " + pp_utt + " [SEP]"
      tokenized_text = tokenizer.tokenize(marked_text)
      indexed_tokens = tokenizer.convert_tokens_to_ids(tokenized_text)
      tokens_tensor = torch.tensor([indexed_tokens])
      embeddings= he_model.bert(tokens_tensor).last_hidden_state
      ...
      weight_matrix[utt2idx[pp_utt]] = sentence_embedding.detach().numpy()
...
d_id = 0
print(len(global_speaker_info))
print(len(X_train_d_id))
