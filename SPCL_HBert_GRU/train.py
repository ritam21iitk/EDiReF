...
def train_epoch(model,
    ...
    def calc_loss(sentences, emotion_idxs, labels):
        ...
        # print(sentences.size())
        ccl_reps = inner_model.gen_f_reps_ours(sentences)
...
def train(model, train_dialogues, dev_dialogues, test_dialogues):
    ...
    devset = data2transformerencoder(dev_dialogues)
    testset = data2transformerencoder(test_dialogues)
    ...
    for epoch in range(CONFIG['epochs']+1):
        ...
        trainset = data2transformerencoder(train_dialogues, train=True)
...
def test(model, data, centers, centers_mask, desc=''):
    ...
    direct_class_report = classification_report(y_true_list,direct_list[idx])
    direct_conf_mat2 = confusion_matrix(y_true_list,direct_list[idx])

    print(direct_class_report)
    print("Direct Confusion Matrix: \n",direct_conf_mat2)


    print('cluster f1 cls {:.4f}'.format(*cluster_f1_scores),
          flush=True)
    cluster_class_report = classification_report(y_true_list,cluster_list[idx])
    cluster_conf_mat2 = confusion_matrix(y_true_list,cluster_list[idx])

    print(cluster_class_report)
...
if __name__ == '__main__':
    ...
    if args.test:
        ...
        if args.task_name == 'meld':
            test_dialogues = load_meld_turn(test_data_path)
        model, centers, centers_mask = load_latest('test/diyi/temp/tmpbrliia0i')    
        testset = data2transformerencoder(test_dialogues)
